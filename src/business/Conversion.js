...
const Conversion = ({ title = "Conversion List", pSize = 10, showHeader = true }) => {
    const [searching, setSearching] = useState(false);
    const [currentPage, setCurrentPage] = useState(1);
    const [conversionList, setUserLists] = useState([]);
    const [updateTable, setUpdateTable] = useState(false);
    const [pageSize, setPageSize] = useState(pSize)
    const [showModal, setShowModal] = useState(false)
    const [showAlert, setShowAlert] = useState(false)
    const [modalTitle, setModalTitle] = useState("Creat Conversion")
    ...
    const currentTableData = useMemo(() => {
        ...
        // if (searching) {
        //     return conversionList.slice(firstPageIndex, lastPageIndex);
        // } else {
        //     return conversionList.slice(firstPageIndex, lastPageIndex);
        // }
    ...
    const toggleModal = (mTitle = "Create Conversion") => {
    ...
    return (
        <Fragment>
            ...
            {showModal &&
                <RightModal title={modalTitle} size={400} toggleModal={toggleModal}>
                    <form onSubmit={handleInputForm}>
                        ...
                        <div className={allStyles.input__group}>
                            <input type="text" id="conversionName" name='conversionName' className={allStyles.input__control} required />
                            <label htmlFor='conversionName' className={allStyles.input__label}>Conversion</label>
                        </div>
            ...
            <div className={`${allStyles.card} ${styles.conversion}`} style={{ overflowX: "auto" }}>
                {showHeader &&
                    <Header title={title}>
                        ...
                        <button style={{ padding: "0px 7px" }} onClick={() => toggleModal("Create Conversion")}>
