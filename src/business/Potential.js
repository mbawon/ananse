...
const Potential = ({ title = "Potential List", pSize = 10, showHeader = true }) => {
    const [searching, setSearching] = useState(false);
    const [currentPage, setCurrentPage] = useState(1);
    const [businessList, setUserLists] = useState([]);
    const [updateTable, setUpdateTable] = useState(false);
    const [pageSize, setPageSize] = useState(pSize)
    const [showModal, setShowModal] = useState(false)
    const [showAlert, setShowAlert] = useState(false)
    const [modalTitle, setModalTitle] = useState("Creat Potential")
    ...
    const currentTableData = useMemo(() => {
        ...
        // if (searching) {
        //     return businessList.slice(firstPageIndex, lastPageIndex);
        // } else {
        //     return businessList.slice(firstPageIndex, lastPageIndex);
        // }
    ...
    const toggleModal = (mTitle = "Create Potential") => {
    ...
    return (
        <Fragment>
            ...
            {showModal &&
                <RightModal title={modalTitle} size={400} toggleModal={toggleModal}>
                    <form onSubmit={handleInputForm}>
                        ...
                        <div className={allStyles.input__group}>
                            <input type="text" id="businessName" name='businessName' className={allStyles.input__control} required />
                            <label htmlFor='businessName' className={allStyles.input__label}>Potential</label>
                        </div>
            ...
            <div className={`${allStyles.card} ${styles.business}`} style={{ overflowX: "auto" }}>
                {showHeader &&
                    <Header title={title}>
                        ...
                        <button style={{ padding: "0px 7px" }} onClick={() => toggleModal("Create Potential")}>
